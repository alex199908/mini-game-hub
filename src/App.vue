...
<template>
  <div class="app-frame">
    ...
    <Transition name="screen" mode="out-in">
      <main v-if="!activeGame" key="home" class="home-view">
        ...
        <section class="game-grid" aria-label="遊戲清單">
          <GameCard v-for="(game, index) in games" :key="game.id" :game="game" :index="index" @select="selectGame" />
        </section>
      </main>

      <GameShell v-else :key="activeGame.id" :eyebrow="`${activeGame.category} / ${String(games.indexOf(activeGame) + 1).padStart(2, '0')}`" :title="activeGame.title" :subtitle="activeGame.description" @back="goHome">
        <template #actions><span v-if="activeGame.status === 'ready'" class="live-badge">LIVE</span></template>
        <Transition name="game-view" mode="out-in" appear>
          <component :is="activeComponent" v-if="activeComponent" :key="activeGame.id" />
          <ComingSoon v-else key="coming-soon" :game="activeGame" />
        </Transition>
      </GameShell>
    </Transition>
  </div>
