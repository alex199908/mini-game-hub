...
function reset() { clearPour(); tubes.value = clone(initial); selected.value = -1; history.value = []; }
...
function clickTube(index) {
  ...
  if (available > 0 && (!target.length || target.at(-1) === color)) {
    ...
    clearPour();
    pouring.value = { source: selected.value, target: index, token: pouring.value.token + 1 };
...
<template>
  <div class="game-panel">
    ...
    <div class="tubes">
      <button v-for="(tube, index) in tubes" :key="index" class="tube" :class="{ selected: selected === index, 'tube--source': pouring.source === index, 'tube--target': pouring.target === index, 'tube--complete': tube.length === capacity && tube.every((color) => color === tube[0]) }" @click="clickTube(index)">
