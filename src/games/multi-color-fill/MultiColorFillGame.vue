...
function reset() { board.value = makeBoard(); moves.value = 0; }
...
<template>
  <div class="game-panel">
    ...
    <div class="fill-board"><span v-for="(color, index) in board.flat()" :key="index" class="fill-cell" :style="{ '--c': colors[color] }" /></div>
    <div class="color-palette"><button v-for="color in palette" :key="color" class="palette-dot" :style="{ '--c': colors[color] }" :aria-label="color" @click="fill(color)" /></div>
