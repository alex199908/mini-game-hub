...
<template>
  ...
  <div v-else class="game-panel">
    ...
    <div class="last-play"><strong>桌面牌</strong><br />{{ last ? `${last.rank}${last.suit}` : "尚未出牌" }}</div>
    <p class="notice">{{ message }}<br />第一版先做單張出牌，牌型引擎下一階段接入。</p>
    <div class="cards"><button v-for="card in human.cards" :key="card.value" class="card" :class="{ red: card.color === 'red', selected: selected[0]?.value === card.value }" @click="toggle(card)"><strong>{{ card.rank }}</strong><span>{{ card.suit }}</span></button></div>
